...
**最新進度請直接跳到檔案最末章節「2026-09-02 收工｜本日總結（六批工作，全部已上線）」。**
...
- 最新 Preview `dpl_9fsDzBLgzVcJXDbD8MSRpdxZTNUi` 已 READY；以 Vercel CLI bypass 驗證 `/`、`/admin-login` 為 200，未授權 admin/face GET 為 401，旗標關閉的 face POST 為 404。
...
1. 使用具名 admin 登入 Preview，驗證知識庫 CRUD 與 28 張草稿卡。
...
**已部署，程式碼與 DB 終於對齊。** `main` = `origin/main` = `0782bbd`，工作樹乾淨。
08-31 與 09-01 章節裡「DB 走在程式碼前面」的落差已經關閉。

起因：使用者登入 `https://www.xunfeng.tw/admin` 看不到「老師服務／案例課程」選單。
查證後不是權限問題（`306465@gmail.com` 在正式庫 `role=admin`），
是正式站 `/api/site-content` 回 404 —— 程式碼從未部署。使用者裁示兩批一起上。

### 二、已完成

兩個 commit，直接推 `main`（Vercel 自動部署，約 75 秒）：

- `e6590bc` feat(referrals,face)：8/31 那批（分潤歸因、面相報告永久化、`/api/plans` 過濾 `^e2e_`）
- `0782bbd` feat(admin)：9/01 網站內容 CMS

### 三、驗證結果（皆為正式站實測）

- `/api/site-content` → 200，服務 9／案例 4／課程 4／主打「掌中訣」active=true
- `/admin/site-services`、`/admin/site-cases` → 200
- `/api/admin/site-content?type=services` 未登入 → 401
- 瀏覽器實測 `/courses`：**`.course-promo-card` 只有 1 張、`#zzjStaticFallback` 已被移除**、
  課程卡 4 張全部來自 DB。這是本次最擔心的那個點（下架後前台仍看得到寫死舊卡），已確認正常。
- `/services` 9 張價格卡、`/cases` 4 張案例卡 + 7 張照片，皆從 DB 渲染
- `/api/plans` → 只回 `basic`／`pro`／`vip`，`e2e_card_test` 不再裸露（08-31 待辦第 2 項結案）

### 四、尚未完成 / 風險

1. **後台介面仍未用真人帳號實際操作過。** 頁面回 200、API 401 正確，
   但「新增一筆→上傳圖片→上架→前台確認」這條路徑是在本機用 `X-Admin-Key` 驗的，
   正式站上沒有點過。請實際跑一輪，特別是圖片上傳與主打課程「儲存並下架」。
2. 分潤功能一樣零實跑（建夥伴、`?ref=` 歸因、後台明細）。
3. `e2e_card_test` 仍 `is_active=true`，信用卡真刷 E2E 未做；驗完必須停用。
4. 面相手機實測、67 條規則回 PDF 對帳、Resend 網域驗證、ZDR 皆原封不動。

### 五、待辦優先順序

1. 後台實跑「網站內容」一輪（見上方風險 1）。
2. 信用卡真刷 NT$1 驗四件事，驗完停用 `e2e_card_test`。
3. 分潤實跑。

### 六、下次起手式

1. 讀本章節。
2. `git status --short --branch`（應為 `main` = `origin/main` = `0782bbd`，工作樹乾淨）。
3. 直接做上方待辦，不需要先補部署。

### 七、Git 狀態

`main` = `origin/main` = `0782bbd`，已 push、已部署，工作樹乾淨。
...
**已部署。** `main` = `origin/main` = `42a6b78`，工作樹乾淨。

### 二、已完成

使用者回饋：側邊欄 5 個區塊、22 個連結全部攤開陳列，畫面凌亂；
並指定「網站內容、顧問服務要移到營運總覽底下，四象與面相屬系統維護模組放最下面」。

- `app/admin/_shell.tsx`：
  - 區塊順序改為 **營運總覽 → 網站內容 → 顧問服務 → 四象問天機 → 面相系統**
  - 區塊標題改成折疊開關（`<button aria-expanded>`），抽出 `isActive()` 共用
  - 預設只展開「目前所在的那一區」；使用者的展開／收合選擇存 localStorage
    （key `xunfeng_admin_nav_open`），換頁保留。無痕模式存不了就靜默略過
  - 收合狀態下若目前頁面就在該區，標題旁顯示綠點；標題右側顯示該區連結數
- `app/admin/admin.css`：`.admin-nav-toggle`／`.admin-nav-caret`（展開時旋轉 90°）／
  `.admin-nav-dot`／`.admin-nav-count`／`.admin-nav-items`（內縮 + 左側導引線）

### 三、驗證結果

因為後台要登入才看得到，且**沒有也不該去拿使用者的密碼**，改用等效做法驗證：
把真實的 `admin.css` 與同構的 nav DOM 兜成一個獨立預覽頁，用瀏覽器實跑。

- 視覺：區塊順序正確、收合態只剩 5 行標題、展開態有內縮導引線
- 互動：`aria-expanded` 由 `["true","true",...]` → 點擊後 `["true","false",...]`，
  `.admin-nav-group.open` 數量正確，收合後綠點出現
- `npx tsc --noEmit` exit 0、`npm run build` ✅
- 部署後比對正式站產物：`/_next/static/css/71bb43b802e23029.css` 含 `admin-nav-toggle`
  （與本機 build 同一個 hash）；`layout-8cf323e4ad9836aa.js` 內區塊順序為
  營運總覽／網站內容／顧問服務／四象問天機／面相系統，且含 `xunfeng_admin_nav_open`

### 四、尚未完成 / 風險

1. 仍未用真人 admin 帳號在正式站點過後台（介面驗證是用等效預覽頁做的）。
   請登入後看一眼側邊欄，並實跑「網站內容」的新增→上傳圖片→上架流程。
2. 其餘遺留同前一章節：分潤零實跑、信用卡真刷未做、`e2e_card_test` 仍 active 待停用。

### 五、待辦優先順序

同前一章節（後台實跑 → 信用卡真刷 → 分潤實跑）。

### 六、下次起手式

`git status --short --branch`（應為 `main` = `origin/main` = `42a6b78`，工作樹乾淨）。

### 七、Git 狀態

`main` = `origin/main` = `42a6b78`，已 push、已部署，工作樹乾淨。
...
## 2026-09-02（第四次）｜實測訪客推廣連結購買路徑，補兩個小坑

### 一、目前狀態

**已部署。** `main` = `origin/main` = `c238920`（＋本篇紀錄 commit），工作樹乾淨。

### 二、做了什麼

使用者問「訪客可以透過推廣連結完成購買嗎？」，於是把整條路徑逐段實測，
而不是看程式碼推論。用正式站上真實存在的推廣碼 `ran81127`（賴仁豪，20%）。

**逐段驗證結果（正式站瀏覽器實跑）：**

| 步驟 | 結果 |
|------|------|
| `/?ref=ran81127` 寫入 cookie | ✅ `xf_ref=ran81127` 確實寫入 |
| 走到 `/member-pricing` cookie 還在 | ✅ |
| 三個方案卡正常（980／1980／4980） | ✅ |
| 訪客按「立即購買」 | ✅ 導到 `/login?next=%2Fmember-pricing`，**不顯示**「登入已過期」 |
| 到 `/login` 後 cookie 還在 | ✅ |
| 「建立新會員」切出註冊表單、`next` 保留 | ✅ |
| 註冊 API 活著 | ✅ 格式錯誤回 400；已存在的 Email 回 409 |
| 建立訂單會寫入歸因 | ✅ 程式路徑確認：`orders/create` 讀 cookie → `resolveReferral` → 寫進 orders |

**驗證中發現並修掉的兩個坑：**

1. 已註冊過的 Email 再次註冊時，直接把 Supabase 的英文原文吐出來
   （`A user with this email address has already been registered`，HTTP 422）。
   正在買方案的訪客看不懂也不知道下一步。改成中文並指路去登入／忘記密碼，回 409。
2. `handlePurchase` 確認 session 後沒有把拿到的 member 餵回 `cachedMember`，
   剛換發完那一刻發票抬頭與 Email 可能是空白，使用者得自己重打。已修。

### 三、結論（誠實版）

**除了「真的送出註冊」與「真的刷卡」這兩步，其餘每一段都在正式站實跑通過。**

這兩步我做不到也不該做：建立帳號要輸入密碼、刷卡要真的付錢。
所以正確的說法是「路徑上找不到已知的阻擋點」，**不是「已確認訪客買得到」**。

### 四、尚未完成 / 風險

1. **請本人用一組全新的 Email 實跑一次完整流程**（推廣連結 → 註冊 → 購買 → 綠界 → 回站看點數）。
   這是唯一能確認的方法。建議先用 `?plan=e2e_card_test` 的 NT$1 方案。
2. 至今 `POST /api/auth/register` 在 24 小時內是 0 筆 ——
   **訪客註冊這條路從來沒有真人跑過**，語意上「沒有已知阻擋點」與「跑得通」還是有差距。
3. 註冊成功後會回到 `/member-pricing`，但**需要再按一次「立即購買」**。
   目前不會自動接續結帳。若要更順，可在 `?next=` 帶上方案代碼回來後自動開發票表單 —— 尚未做。
4. 其餘遺留不變：信用卡真刷未做、`e2e_card_test` 仍 active 待停用、
   後台「網站內容」尚未真人實跑。

### 五、待辦優先順序

1. 用全新 Email 實跑訪客購買全程（含刷卡）。
2. 驗完停用 `e2e_card_test`。
3. 後台「網站內容」真人實跑。

### 六、下次起手式

`git status --short --branch`（應為 `main` = `origin/main` = `c238920` + 本篇紀錄，工作樹乾淨）。

### 七、Git 狀態

`main` = `origin/main` = `c238920` + 本篇紀錄 commit，已 push、已部署，工作樹乾淨。
...
## 2026-09-02（第五次）｜推廣連結先導註冊 + 註冊即終身綁定推廣人 + 註冊表單簡化

### 一、目前狀態

**已部署。** `main` = `origin/main` = `f208739`（＋本篇紀錄 commit），工作樹乾淨。

### 二、需求與決策

需求：「點開推廣連結要先引導完成會員註冊，這樣才能綁在這個推廣人的帳號底下，
註冊方式可以簡化為手機註冊+email，這樣從推廣者進入的會員消費都可以認列在這個推廣者的帳號底下。」

問過使用者兩個會改變作法的分歧點，裁示如下：
- 註冊方式 → **欄位簡化、保留密碼**（不接簡訊 OTP，零成本、當天可上）
- 進站行為 → **直接強制導到註冊頁**（不是提示條）

### 三、已完成

**1. 歸因改為「註冊綁定優先」（這是需求的核心）**

原本只看 `?ref=` cookie（90 天 last-touch）。清 cookie、換裝置、
或中途點到別人的推廣連結，業務就白帶了。

現在 `orders/create` 走新的 `referralFieldsForOrder()`：
先讀 `profiles.referral_code`（註冊當下綁的那位），沒有才退回 cookie。
**註冊那一刻就終身歸戶。** 既有會員沒有 referral_code，維持原本的 cookie 行為。

課程報名（`courses/checkout`）沒有會員身分，仍用只看 cookie 的舊函式。

**2. 未登入訪客帶 `?ref=` 進站 → 直接導註冊頁**

`ReferralCapture` 改寫：寫 cookie 之後，若未登入且不在排除清單
（`/login`、`/reset-password`、`/admin`、`/admin-login`、`/thanks`），
`router.replace("/login?tab=register&ref=CODE&next=原頁path")`。

- 已登入者不攔（早就綁定過了，攔他只是擋路）
- `next` **只帶 path 不帶 query**，否則註冊完導回去又會被攔成無窮迴圈
- 註冊頁顯示「您是由 OOO 推薦加入」，新增公開 `GET /api/referral/lookup?code=`，
  **只回名字**，不吐分潤比例、聯絡方式或內部 id（這支完全公開，任何人都能猜代碼來打）

**3. 註冊表單簡化為「手機 + Email + 密碼」**

- 拿掉姓名欄位；手機改**必填**並正規化（吃掉 `+886`、空白、連字號，統一存 `09xxxxxxxx`）
- cookie 被瀏覽器擋掉時，註冊頁把網址上的 `?ref=` 一併送進 API 當備援
- 加上「註冊即送 30 點」的說明

### 四、驗證結果

正式站：
- `GET /api/referral/lookup?code=ran81127` → `{"ok":true,"partner":{"name":"賴仁豪"}}`；
  不存在的代碼與注入字串都回 `{"ok":false,"partner":null}`
- 註冊手機驗證：沒填 → 400「請填寫手機號碼」；`1234` → 400「手機號碼格式不正確…」；
  `+886 912-345-678` 正規化通過（停在 409 已註冊，**沒有建立任何帳號**）
- 瀏覽器實跑 `https://www.xunfeng.tw/?ref=ran81127`：
  自動導到 `/login?tab=register&ref=ran81127&next=%2F`，
  註冊分頁已開啟、cookie 已寫入、欄位只剩 phone/email/password（無 name）、
  banner 顯示「您是由 賴仁豪 推薦加入，完成註冊後即綁定這位推廣夥伴。」
- 已登入者帶 `?ref=` 進站**不會**被導走（本機驗證）
- `npx tsc --noEmit` exit 0、`npm run build` ✅、`npm run test:unit` 169 passed

### 五、尚未完成 / 風險

1. **「註冊 → 訂單歸到該推廣人」這條完整鏈路沒有真人跑過。**
   我沒有也不該建立帳號，所以 `profiles.referral_code` 寫入與
   `referralFieldsForOrder` 的優先序只有程式與型別層面的保證。
   **請用全新 Email 從 `?ref=ran81127` 註冊一次，然後下一筆單，
   到 `/admin/referrals` 看有沒有歸到賴仁豪名下。**
2. **不再收集姓名的副作用**：結帳時發票抬頭不會自動帶入，使用者要自己填；
   後台會員管理的姓名欄位會是空的，改以 Email／手機辨識。若不能接受要回頭加。
3. **強制導向的副作用**：推廣連結分享到社群後，每個未登入訪客都會被彈到註冊頁，
   看不到首頁內容。這是使用者明確選的（綁定率優先），但如果發現跳出率太高，
   改成「首頁置頂引導條」只要改 `ReferralCapture` 一個分支。
4. 已註冊過的人若登出後再點推廣連結，會被導到註冊頁、按下去得到
   「這個 Email 已經註冊過了，請直接用它登入」，需要自己切到登入分頁。可接受但不夠順。
5. 其餘遺留不變：信用卡真刷未做、`e2e_card_test` 仍 active 待停用、
   後台「網站內容」尚未真人實跑。

### 六、待辦優先順序

1. 用全新 Email 從推廣連結註冊 → 下單 → 驗歸因（最重要，這是本次需求的驗收點）。
2. 信用卡真刷，驗完停用 `e2e_card_test`。
3. 後台「網站內容」真人實跑。

### 七、Git 狀態

`main` = `origin/main` = `f208739` + 本篇紀錄 commit，已 push、已部署，工作樹乾淨。
...
## 2026-09-02（第六次）｜下單路徑伺服器端實測 + 註冊頁推薦人文案調整

### 一、目前狀態

**已部署。** `main` = `origin/main` = `d191251`（＋本篇紀錄 commit），工作樹乾淨。

### 二、已完成

**1. 「推廣連結到底能不能下單」——伺服器端實測（不是推論）**

寫了一支一次性的 vitest 檔，把 `/api/orders/create` 的伺服器流程原封不動跑一遍
（唯一跳過的是 bearer 驗證），打的是**正式資料庫**。四項全過：

- 980 基礎方案查得到、`is_active`、TWD
- 推廣碼 `ran81127` 查得到、啟用中、`commission_rate = 0.2`
- **訂單真的寫得進 orders**，`referral_partner_id`／`referral_code`／`referral_rate`
  三個欄位都正確寫入；980 × 20% = **196 元** 分潤算得出來
- 綠界 `createCheckoutParams` 產得出 64 碼 hex 的 `CheckMacValue`，action URL 是 https

驗證用訂單（`order_no` 前綴 `VERIFY`）已刪除，事後查 DB 為 0 筆，沒留痕跡。
**驗證檔已刪除**（曾短暫放在 `lib/_verify/order-path.test.ts`，不進 repo）。

結論：**除了「按下建立帳號」那一個動作，整條下單路徑每一段都驗過且通過。**

**2. 註冊頁推薦人文案**

「您是由 OOO 推薦加入，完成註冊後即綁定這位推廣夥伴。」
→「您是由 OOO 推薦加入，趕快來完成註冊吧。」
（原句在講系統內部行為，對訪客沒有意義。）

### 三、驗證結果

- 正式站產物比對：`login/page-f847f5c1ebc159fe.js` 含新文案 1 次、舊文案 0 次
- 瀏覽器實跑 `https://www.xunfeng.tw/?ref=ran81127`：**500ms 內**導到
  `/login?tab=register&ref=ran81127&next=%2F`，cookie 已寫入，
  banner 顯示「您是由 賴仁豪 推薦加入，趕快來完成註冊吧。」，欄位只剩 phone/email/password
- `npm run build` ✅

> 過程中有一次取樣顯示沒導向，重測兩次都在 500ms 內正確導向，
> 且部署產物確認含導向邏輯 —— 判定為前一次測試殘留的取樣時序問題，非功能異常。

### 四、尚未完成 / 風險

1. **仍然只差「真人按下建立帳號」這一步。** 我不能建立帳號。
   請用全新 Email 從 `?ref=ran81127` 註冊，走到綠界付款頁即可（不必真的刷）。
   要驗的三件事：註冊送得出去、`profiles.referral_code` 綁到賴仁豪、訂單成立且歸戶。
2. 其餘遺留不變：信用卡真刷未做、`e2e_card_test` 仍 active 待停用、
   後台「網站內容」尚未真人實跑、不再收集姓名的副作用（發票抬頭要自填）。

### 五、待辦優先順序

1. 全新 Email 從推廣連結註冊 → 下單 → 到 `/admin/referrals` 驗歸因。
2. 信用卡真刷，驗完停用 `e2e_card_test`。
3. 後台「網站內容」真人實跑。

### 六、Git 狀態

`main` = `origin/main` = `d191251` + 本篇紀錄 commit，已 push、已部署，工作樹乾淨。
...
## 2026-09-02 收工｜本日總結（六批工作，全部已上線）

> 這一節是**當天總結**，取代逐段翻閱上面六個章節。細節仍在各章節裡。

### 一、目前狀態

`main` = `origin/main` = `07fda5c`，**工作樹乾淨，全部已部署**，正式站健康檢查全綠：
`/`、`/services`、`/cases`、`/courses`、`/member-pricing`、`/login`、`/admin/site-services` 皆 200；
`/api/site-content`、`/api/plans`、`/api/referral/lookup` 皆 200。

**程式碼與 DB 已對齊**，不再有「DB 走在程式前面」的落差（那是 08-31 遺留，今天早上關閉的）。

### 二、本日做了什麼（依序）

1. **後台網站內容 CMS**（老師服務／案例課程）—— 從 `content/*.json` 搬進 Supabase，
   可新增／編輯／上架下架／排序／上傳圖片。JSON 降級為 fallback。
2. **部署 8/31 + 9/01 兩批**，順手關掉 `/api/plans` 裸露 `e2e_card_test` 的問題。
3. **後台側邊欄改可折疊並重排**：營運總覽 → 網站內容 → 顧問服務 → 四象 → 面相。
   預設只展開目前所在區塊。
4. **修掉「結帳時登入已過期」**（本日最重要的 bug）。根因：Supabase access token
   只有 1 小時，而前台從來沒存過 refresh token。新增 `/api/auth/refresh`、
   `MemberSessionKeeper`（背景保鮮）、`XFSession`（401 自動換發重送），
   並在結帳開發票表單「之前」先確認 session。
5. **推廣連結改為先導註冊 + 註冊即終身綁定**：`orders/create` 歸因改成
   「`profiles.referral_code` 優先，cookie 為輔」；未登入訪客帶 `?ref=` 直接導註冊頁；
   註冊表單簡化為手機 + Email + 密碼。
6. **下單路徑伺服器端實測** + 註冊頁推薦人文案改成「趕快來完成註冊吧」。

### 三、最重要的一件未完成事項

**「真人按下建立帳號」這一步始終沒有被做過。** 我不能建立帳號，所以：

- `profiles.referral_code` 實際寫入
- 註冊後的訂單真的歸到賴仁豪名下

這兩件事只有程式與型別層面的保證。其餘每一段（方案查詢、推廣碼解析、訂單 insert
含三個歸因欄位、980×20%=196 分潤、綠界簽章、前端導向與文案）都已在正式環境實測通過。

**下次開工第一件事：用全新 Email 從 `https://www.xunfeng.tw/?ref=ran81127` 註冊，
走到綠界付款頁即可（不必真的刷），然後到 `/admin/referrals` 確認歸戶。**

### 四、其餘待辦（優先順序）

1. 上述訪客註冊 → 下單 → 歸因驗收。
2. 信用卡真刷 E2E（`?plan=e2e_card_test` 的 NT$1 方案），**驗完必須停用**
   （`update public.plans set is_active=false where code='e2e_card_test';`）。
3. 後台「網站內容」真人實跑一輪（新增→上傳圖片→上架→前台確認；
   主打課程「儲存並下架」後 `/courses` 大卡要消失）。
4. 不再收集姓名的副作用：發票抬頭要自填、後台姓名欄位會空。若不接受要回頭加。
5. 強制導向註冊的副作用：推廣連結分享出去後，未登入訪客看不到首頁內容。
   若跳出率太高，改成「首頁置頂引導條」只需改 `ReferralCapture` 一個分支。
6. 更早的遺留：發票走 EZPay 仍沙箱、Resend 網域驗證未做、council 待補
   OpenAI/Gemini/DeepSeek key、面相 67 條規則回 PDF 對帳、ZDR 簽署。

### 五、下次起手式

1. 讀本節（不必翻上面六個章節，除非要細節）。
2. `git status --short --branch` → 應為 `main` = `origin/main` = `07fda5c`，乾淨。
3. 直接做第三節那件事。

### 六、Git 狀態

`main` = `origin/main` = `07fda5c` + 本節紀錄 commit。本日共 10 個 commit，全部已 push、已部署。

### 七、長時間程序

本專案無。（機器上有一個 `next dev` 在跑，但屬於另一個專案
`~/Downloads/菁英班-系統開發/app`，與巽風無關，未動它。）
