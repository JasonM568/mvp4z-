...
三件事合成一個 commit `8cfee15` 推上 `main`（15 檔、+678 / −74），
Vercel 自動建 production `dpl_Gz5nj3pHtQgxgX1iZHNHEEALxHND`，build 56 秒 → READY，
alias 已切 `www.xunfeng.tw` / `xunfeng.tw`。

部署探針用的是首頁文案：這次把「先看方向」那行改成七種決策，
所以輪詢正式站首頁出現「宜借力推進」即代表新版已生效，第 4 次輪詢（約 1 分鐘）命中。

**還沒做真人驗收。** 首頁文案只證明 build 有出去，證明不了報告本身。
下次開工第一件事是實跑一份報告，重點看兩件本機測不到的事：
1. 老師那份 3741 字文件是否真的進了 prompt（查 `council_runs` 的 prompt 長度差，
...
### 附記：最初始設計 repo 的調查（2026-09-06）

使用者提供 https://github.com/kingking12180823-ui，問最初始設計裡有沒有批盤指令或邏輯。

該帳號兩個 repo：
- `xunfeng-yixue-system`（179KB，19 檔，單一 commit）＝ 本系統的最初始設計
- `xunfeng-official`（84 檔，全是 HTML／圖片／影片）＝ 純靜態官網，無 lib/、無 app/api/

**結論：沒有批盤邏輯，也沒有批盤指令。**

- 全 repo 零術數常數表（無天干、地支、八卦、六十四卦）
- `route.ts` 的函式全是 prompt 組裝、品質門檻與兜底，無術數計算
- prompt 只規定「輸出哪十段」，從不說「怎麼排盤／裝卦」。
  最接近的一句是終稿 prompt 的「若出生時辰或節氣資料不足，必須標明降權」，
  那是保守話術不是起卦規則

**原作者自己知道，且寫下來了** —— `index (2).html:67`：

> 目前前端版是「策略報告引擎」，不是正式排盤引擎。正式營運前，
> 四柱、奇門盤、六爻納甲與梅花體用仍要接專業排盤核心。

這句點名的四項，四柱已由 `lib/yixue/` Phase 0 完成，其餘三項正是目前缺口。
先前提給使用者的建置順序（梅花 → 六爻 → 奇門）與這句話一致。

**沿用至今的部分**（這個 repo 是現行 prompt 架構的祖先）：
- 十段式報告骨架 → 現在的 `reportSkeleton`
- 四個分身（主判讀／策略推演／攻防反證／定稿）→ 現在的四個 persona
- `baseRules` → 現在 `defaults.ts` 的共同規則
- `cleanReportText`／`buildQualityGate`／`hasUsableFinal`／`buildSafeFallbackReport` → 仍在跑
- 「可進、可試行、暫緩、或需重新起局／補資料」→ 今天擴成七種的那組決策型態的源頭，
  `normalizeDecision` 的舊詞對照表對的就是這幾個詞

**解掉一個懸案**：先前記錄的「終稿人設寫 7/14/30 KPI、actionPlan 寫 3/7/30」落差，
源頭是最初始 repo 裡兩份文件本來就打架 —— `lib/personas.ts` 終稿格式第 8 條寫
「7日、14日、30日 KPI」，`README.txt` 第 4 點寫「3/7/30日行動」，一路抄到現在。
使用者實際看到的報告照 `reportSkeleton` 走，所以應留 3/7/30、改掉人設那句。

### 發現：老師的流派決策停在草稿，一個月未生效（2026-09-06）

使用者問「要如何讓老師把排盤邏輯或 Prompt 從後台提供，不要再經過我」。
查後台權限與各設定表時發現：**機制早就做好，老師也用過一次，但停在草稿。**

`admin_audit_logs` 只有一筆相關紀錄：

```
2026-08-10 01:29:29 UTC（台灣 09:29）
kingking0909@yahoo.com.tw
school_settings.save_draft  →  ai_school_profiles b8655fce…
lateZiDayPillar: "same"
```

程式預設 `fengyi-v1` 是 `"next"`（進位），老師改成 `"same"`（不進位），
按了儲存草稿後沒有下一步。`loadSchool()` 只讀 `status='published'`，
`ai_school_profiles` 至今 published = 0，所以**這一個月每份報告仍用「進位」排**。

影響（依 SCHOOL-DECISIONS.md 決策 1 的盤例，2024-01-01 23:30 生）：
現在跑的日柱是乙丑，老師草稿的意思是甲子。日柱是日主所在，
直接決定十神、旺衰、用神。**老師已表達專業判斷，系統沒採用，他可能以為採用了。**

與今天修的文件 bug 同一種病：畫面顯示狀態、實際沒生效、沒有人被告知。

**待辦：先去問老師「不進位」是不是他的決定，是的話請他自己按發布。**
這一按會改變往後所有晚子時個案的判讀基礎，不能由工程或營運代按。

三頁老師都已有權限（email 在 ADMIN_EMAILS 登入即自動升 admin）：
`/admin/prompt-settings`、`/admin/documents`、`/admin/school-settings`。

**排盤邏輯本身不開放後台上傳，這是刻意的**：流派選擇（專業判斷）走後台，
演算法（程式）不走。`lib/yixue` 必須是純函式才能被 golden case 逐欄位比對
（`guard.test.ts` 在管），演算法一旦進 DB 就沒有東西能驗證盤是對的，
已收費報告也無法重現。老師負責「選」、工程負責「算」，可選範圍隨各術 Phase 擴大。

**要真正不經過營運，建議補三件（皆小）：**
1. 草稿存超過 N 天未發布 → 寄信給老師與管理者。
   `lib/notifications/admin-alerts.ts` 已有寄信基礎設施。
2. 後台一律顯示「實際生效什麼」而非「勾了什麼」。
   `school-settings` 已做對；`documents` 顯示「已納入 3741 字」卻從未進 prompt，
   正是因為顯示的是勾選狀態；`prompt-settings` 同樣要補。
3. 按鈕權重反了：現在「儲存草稿」是主按鈕、「發布草稿」是 ghost。
   草稿與已發布不一致時，發布應升為主按鈕並帶警示。

### 後台生效狀態透明化 ＋ 草稿未發布提醒（2026-09-06）

針對前一節發現的「老師的流派草稿放一個月未生效、沒有人知道」，做了三件補救。
共同根因是**後台顯示的是「存了／勾了什麼」，不是「報告真的用了什麼」**。

**b. 生效狀態一律問報告管線本身（核心改動）**

新增 `GET /api/admin/effective-settings`。它刻意呼叫 `loadPromptSettings()` 與
`loadSchool()` —— 報告管線用的同一組 loader —— 而不是自己查表自己判斷。
讀之前先 `invalidate*Cache()`，因為後台要看的是現在的真相不是 60 秒前的印象。

新增共用元件 `app/admin/_effective-status.tsx`（`useEffectiveStatus` +
三個顯示元件）。**這個元件不做任何自己的推算**，一旦開始推算，同樣的落差會再長回來。

三頁接上並移除各自的推算：
- `/admin/documents`：原本「目前納入 Prompt 的字數」是前端從勾選狀態加總的，
  正是那個顯示 3741 字卻一次都沒進 prompt 的元凶。現在改顯示
  `block_chars`（documentBlock 實際長度），並把舊那張卡改名為
  「目前勾選的字數（這是勾選狀態，不代表已進入 Prompt）」。
  兩個數字都留著，因為它們不一致正是要被看見的訊號。勾選／刪除後會重問。
- `/admin/prompt-settings`：移除自行推算的「目前生效版本／草稿」兩張 KPI 卡。
- `/admin/school-settings`：同上；另外會逐條列出草稿與生效值的差異。

**c. 按鈕權重跟著狀態走**

原本「儲存草稿」固定是主按鈕、「發布草稿」固定是 ghost，老師按完主按鈕就離開，
完全合理。現在有未發布且真的有差異的草稿時，發布升為主按鈕、文案改成
「發布草稿（尚未生效）」，儲存降為次要。prompt-settings 有未存修改時仍以「先存」為主。

**a. 每天檢查未發布草稿並寄信**

`GET/POST /api/cron/pending-drafts`，`vercel.json` 排 `0 1 * * *`（台灣 09:00）。
收件人走 `ADMIN_ALERT_EMAILS || ADMIN_EMAILS` —— 老師的信箱本來就在裡面，
所以提醒直接到他手上，不必經過營運轉達。
- 放超過 3 天才催（當天存的還在編輯中）
- 同一份草稿最多 7 天催一次，去重狀態記在 `admin_audit_logs`
  的 `pending_draft.alerted`，不另外開表
- 只有真的寄出去才記，否則寄信失敗會被誤認成已通知而靜默 7 天
- 信裡逐條寫出差異（「晚子時日柱：進位到隔日 → 不進位」），
  收信的人不必登入就能判斷急不急

**抽出兩支純函式並補測試**（route 不該持有領域邏輯）：
- `lib/school-settings/diff.ts` — 草稿與生效值的差異句子。9 個 case，
  其中一條直接用 DB 裡那筆真草稿的內容當輸入，鎖住它會被說出口。
- `lib/admin/pending-drafts.ts` — 該不該催、催信寫什麼。11 個 case，
  含「無作用草稿不催」「剛存的不催」「沒有該催的就回 null 不寄空信」。

驗證：tsc 過、vitest 26 檔 **217 passed** / 2 skipped、next build 過（105 頁）。

**這批沒有做的事**：沒有替老師按任何發布鍵。那筆「不進位」的草稿仍是草稿——
要不要生效是老師的專業判斷，工程與營運都不該代按。

### 2026-09-06 收工

今日四批上線，正式站為 `08e8c5e`。工作區乾淨、與 origin 同步，無長時間程序在跑。

commit：`8cfee15`（文件進 prompt ＋骨架順序＋決策七種）、`3a23271`（改名）、
`eb97707`（後台生效狀態透明化＋草稿提醒），另三筆 docs。
驗證：tsc 過、vitest 26 檔 217 passed / 2 skipped、build 105 頁。

**今日全部是機器驗證，沒有任何一項經過真人跑報告驗收。**
下次第一件事就是實跑一份報告；判斷文件有沒有進 prompt 要看
`council_runs` 的 prompt 長度差，不要看後台字數。

刻意沒做的三件（都不該由工程或營運代做）：
- 沒有代跑報告（要扣 20 點）
- 沒有替老師按任何發布鍵
- 沒有手動觸發 pending-drafts cron（那等於替使用者發出通知，時機該由他選）
...
使用者連問三題：即時問答是 GPT 連結還是串 API、共振度 87 分怎麼算的、
批盤用哪一家模型。查證後浮出真正的問題——**四術裡只有八字是程式排盤**。

`lib/yixue/index.ts` 的 `buildYixueChart()` 讀了四個 modules 旗標，
但函式裡只有 `bazi` 分支有實作。奇門、六爻、梅花是把表單資料原樣丟進 prompt，
`personas.ts` 甚至寫著「請依此時間推算上下卦與動爻」——直接叫模型心算起卦。

同一輪也查出 `structured.ts:125` 的 prompt 範例 JSON 硬寫 `"resonance":87`，
模型照抄。正式庫 29 份報告：87 出現 20 次、85 出現 8 次、75 出現 1 次，
連判「暫緩」的那份都給 85。分數與結論完全脫鉤。

### 做了什麼

**Phase 1 梅花（`e43766a`）**
`gua/trigram.ts` 八卦與六十四卦基礎（三術共用）＋ `meihua/meihua.ts`。
六十四卦表是手打硬資料，測試改用結構性質驗證：卦名前兩字必須等於
上下卦的自然象（「天澤履」＝上天下澤），抄錯一格就會爆。

**Phase 2 六爻（`bd13433`）**
`gua/ganzhi.ts`（干支、沖合、旬空）、`gua/najia.ts`（納甲、京房八宮、六親、六神）、
`liuyao/liuyao.ts`。八宮六十四卦用翻爻規則生成而非手打 192 個欄位——
規則只有 8 條，而且能被古籍卦序驗證。

**Phase 3 奇門（本次）**
`qimen/tables.ts`（二十四節氣三元局數表、九星八門八神、八宮圓周）＋ `qimen/qimen.ts`。
定局採拆補法。跑 192 個時辰的結構不變式掃描時抓到一個真 bug：
時干落中五宮時 `zhiFuPalace` 為 5，而 5 不在八宮圓周上，會排不出盤。

**共鳴度改程式計算**
`lib/ai/council/resonance.ts`，四項真實資料：一致度 0.45、確信度 0.25、
排盤完整度 0.15、排盤覆蓋率 0.15，區間固定 60–90。
prompt 明確要求模型不要輸出 resonance，schema 收下也忽略。
計算依據存進 `structured.resonanceBasis` 並顯示在報告上。
實測：四術判讀分歧得 75、大致相近得 82、全部滿分才 90。

**起卦時間**
六爻的時間起卦原本沒有「現在時間」可選，只能沿用事件時間——使用者反映後補上。
同時修掉 `formatNow()` 讀瀏覽器時區的問題，改用 `Intl` 的 `Asia/Taipei`。
會員人在國外或裝置時區設錯，原本會起到別的時辰的卦而且無跡可循。

### 重要判斷

**裝盤與斷盤分開。** 程式只做有唯一答案的部分。六爻**刻意不給旺相休囚死分數**、
奇門**刻意不做格局判定**——那些需要先定權重模型或取用範圍，屬流派決策。
未經老師拍板就給一個數字，等於把「共鳴度 87」的錯誤換個地方再犯一次。

**奇門必須由老師校對。** 梅花與六爻的每一步都能拿古籍對到底（納甲表、八宮卦序、
六親口訣都有定本），奇門的定局法沒有定本，工程無從自證。已在
SCHOOL-DECISIONS.md 決策 8 附三張完整盤例請老師比對。
仍先上線的理由：現況是 AI 自己想像一個局，沒有任何人能檢查，比這更糟。

**梅花的農曆日跟著曆法流派的晚子時決策走。** 不跟會出現同一份報告裡
「八字算隔日、梅花算當日」——那是兩派都不是。

### 查證附帶結論（回答使用者的三個問題）

- 即時問答是**串 API**（OpenAI `gpt-4.1-mini`），不是 GPT 連結。
  但 `public/content/site.json:13` 還留著舊做法的 `aiUrl`，指向一個公開可用的
  自訂 GPT（實測回 200）。程式沒有任何地方讀它，但 JSON 本身公開可抓，
  等於留了一條免費繞道。**尚未處理，等使用者決定。**
- 批盤用**三家**：第一輪與第二輪 OpenAI＋Gemini＋DeepSeek 平行，
  終稿由 OpenAI 一家獨做。三家 key 都已設好在跑（CLAUDE.md 的「待補 key」是過期資訊）。
  面相模組只有 OpenAI（Gemini vision 有程式但從未在 `face_provider_approvals` 核准）。
- 最初始 repo（`codex-巽風系統/xunfeng-yixue-system`）`index (2).html:67` 寫著
  「目前前端版是策略報告引擎，不是正式排盤引擎……仍要接專業排盤核心」。
  **缺排盤是原始設計就知道的缺口**，不是後來走鐘。該 repo 沒有任何流派定義。
...
- 奇門待老師校對（決策 8）
- 決策 1–7 全部仍未簽核；2026-08-10 的流派草稿仍未發布
- `site.json` 的 `aiUrl` 待處理
- 真人驗收全部未做

### 報告新增「反證」段落＋後台專屬欄位（2026-09-07）

使用者要求「四象報告要增加一個反證的條件，後台要有一個反證的欄位讓老師輸入 Prompt」。

**先釐清的現況**：反證其實早就存在，但只跑在內部——第二輪的「攻防反證分身」
就是這個角色，老師也已能在 `/admin/prompt-settings` 編它的人設。問題是
它的產出**從不出現在交付的報告裡**，讀者看不到反面論證；而且那個欄位是
「你是誰」的人設，不是「要反證什麼」的規則。

**使用者拍板**：報告新增反證段＋後台專屬欄位，**不增加 LLM 呼叫**
（另兩個選項是多跑一輪獨立反證第 8 通、或只加後台欄位不進報告）。
維持 7 通、20 點、產報告時間不變。

**位置**：四象合參之後、時間節奏之前。合參剛把結論收斂起來，緊接著挑戰它，
讀者才會帶著反面意見往下讀行動方案。單術時合參段不出現，反證仍會出現
（一術也可能判錯），中文序號自動接上，快照已驗。

**與關鍵風險的分工**（最容易被寫成同義複述，所以寫進預設內容並加測試鎖住）：
- 風險 = 照這個判斷去做，執行上可能出什麼事
- 反證 = 這個判斷本身可能就是錯的，錯在哪、什麼證據會推翻它

預設要求推翻條件必須具體到能被查證（「若某月營收未達某數」才算數，
「若情況有變」等於沒寫）；找不出有力反證時要直說，不得為了湊滿段落編造反方意見。

改動：`settings/schema.ts`（新增 counterEvidence）、`settings/defaults.ts`（預設內容）、
`settings/render.ts`（插入順序）、`app/api/ai/council/route.ts`（終稿自我檢查清單
擴為 11 條）、`app/admin/prompt-settings/page.tsx`（新增反證編輯卡）。

**相容性（重要）**：`counterEvidence` 刻意給了 `.default()`。這個欄位是今天才加的，
老師先前存的設定沒有它；若設成必填，舊設定會 parse 失敗 → `loadPromptSettings`
回退預設值 → **他整份已發布的編輯靜默失效，而後台不會有任何人察覺**。
那正是本週修的那一類 bug。已加測試鎖住（刪掉 counterEvidence 仍要 parse 成功）。
**日後在既有設定上加欄位，一律要給 default。**
查證時 `ai_prompt_profiles` 仍是 0 筆，所以沒有既有資料受影響。

**刻意沒動**：
- 兜底報告（fallbackReport）未加反證段——資料不足時的安全網不需要反證，
  且動它會改變收費路徑上的兜底輸出。
- 交付門檻 `hasUsableFinal` 的結構錨點未動——那是「≥4 個錨點」的寬鬆檢查，
  把反證加進去只會讓門檻更鬆。反證的要求走終稿自我檢查清單，
  與先前的關鍵點／時間節奏／風險一致。

`prompt-baseline` 報告骨架四則快照刻意更新，測試檔頭已補紀錄。
...
老師反應「有人報名課程但後台看不到」。查證後是兩件事疊在一起，而且兩件都是真的。

**一、後台沒有這一頁。** 側欄的「預約名單」是諮詢預約、「課程上架」是編輯招生頁，
課程報名沒有對應頁面。報名資料只寄生在訂單管理，而報名者填的 LINE、學習背景、
報名動機，只有點進單筆訂單明細才看得到，列表完全不顯示。

**二、5 筆報名 0 筆成交。** 所以在訂單列表上全是「已取消／失敗」，
跟沒人報名長得一樣。細看：4 筆連一筆 payments 紀錄都沒有（沒去付款，
24 小時後被排程自動取消，實測 1450–1485 分鐘），1 筆真的刷了但失敗。

對照組：membership 訂單 6 筆已付款、10 筆取消，所以「填完表跑掉」是兩種訂單共通的
行為，不是課程結帳壞掉。

被漏掉的人：吳淑雯 9/06 03:59 完整填完（有 LINE、有動機）→ 刷卡失敗 →
15:27 用英文名再試一次 → 連綠界都沒到就放棄。同一支手機。她是五個人裡唯一
填了報名動機的，意願最高，而老師不知道有這個人。

### 設計上的關鍵取捨

**跟進狀態與付款狀態分開。** 這一頁若沿用訂單的狀態欄位，或用付款與否當預設過濾，
最該打電話的那幾個人會剛好被濾掉——那正是問題的成因。所以新增獨立的
`contact_status`（待聯繫／已聯繫／已完成報名／已結案），值刻意只留四個，
狀態一多實務上就沒人維護。

`contacted_at` 由後端寫，不接受前端傳入：那是稽核時間點，讓呼叫端自己決定
等於它可以被填成任何值。退回「待聯繫」時一併清空，避免留下誤導的時間戳。

付款與否一律以 `orders.paid_at` 為準，不看報名的 `status`——後者要靠 webhook 回寫，
那條路徑本週才修好，舊資料不一定跟得上。
...
- **報名轉換率本身是另一個問題**：5 筆報名 0 成交、4 筆連付款頁都沒到。
  是招生頁動線、付款流程還是 6000 元的金額門檻，需要另外查
...
三支 API 各說各話，三種文案，而且都只回 `{ error: "..." }` 一個字串。
天機書的錯誤畫面因此只能顯示「系統提示：點數不足，請先儲值或升級方案」，
底下唯一的按鈕是**「返回修改」**。

那顆按鈕對這個錯誤是錯的引導——會員改表單也變不出點數。剛用完 30 點免費體驗、
正準備看第二份報告的人，在這一刻被推進死路。這是整個轉換漏斗最貴的一格。

### 設計上的取捨

**給 code 而不是讓前端比對中文。** 原本若要讓前端認得「這是點數不足」，只能比對
message 字串——改一次文案就壞一次，而且三支 API 文案還都不一樣。所以加
`INSUFFICIENT_CREDITS` 這個 code，連同 required／remaining／shortfall 一起回。
`errorBody()` 對沒有 code 的錯誤行為完全不變，不動到其他路由。

**輸入頁只提示不擋。** 每個方案的 `monthlyFreeQuota` 都是 0，所以前端算得準，
本來可以直接停用按鈕。但餘額是前端拿到的快取值，用它鎖按鈕，萬一值過時就會把
實際有點數的會員關在門外。CLAUDE.md 寫明「前端只顯示狀態，不決定權限扣點」，
所以提示照給、按鈕照開，最終判定留在後端。

### 兩個差點上線的錯

**元件借用了不存在的樣式。** 原本寫 `className="btn ghost"`。查下去發現
`.btn.ghost` 全站沒有定義、`.btn.primary` 只定義在 `styles/member.css`，
而面相頁用的是完全不同的 `.face-primary`。照那樣上線，同一個提示卡在天機書
有樣式、在面相頁是兩個裸連結。改成 `.xf-credits-btn` 自帶樣式。

**CSS 一度放進 `decision.css`。** 面相頁不載入那支，同樣會裸奔。改由元件自己 import。

這兩個都不會讓 tsc 或 build 報錯，只會在特定頁面靜默變醜。核對類名才抓到。

### 驗證

tsc 過、vitest 366 passed / 2 skipped、next build 過（107 頁）、
`node --check` 確認純 JS 版語法無誤。

### 遺留

- 未經真人驗收
- **本次查不到正式庫**（Supabase MCP 這個 session 沒連上），
  所以「目前有幾位會員卡在 0 點」沒有數字佐證
- 更進一步的做法：剩餘點數偏低時就在會員中心主動提示，而不是等他要用時才說


---

## 2026-09-22　點數卡住名單與到期掃描

Supabase MCP 這次接得上，補查了 9/21 交付時缺的那個數字。

### 查證過程中自己犯的錯

第一次查得到「7 位卡在低點數」，寫出去之前又查了一層才發現其中一位的 entitlement
`status='active'` 但 `expires_at` 是 2026-07-16——**早就過期了**。

再查才知道有 7 筆都是這種狀態，最久從 6/01 放到現在，從來沒有東西在掃。
正確答案是 **6 位**。這件事本身就變成了第二個要修的東西。

教訓：`member_entitlements.status` 不可單獨信任，任何統計都要一起比 `expires_at`。
程式本身是安全的（扣點路由都有比），但人與查詢會被騙。

### 6 位卡住的人長得完全一樣

註冊 30 → 產 1 份報告扣 20 → 剩 10。六位一模一樣，不是 9 也不是 11。
有付費 0、開過訂單 0、用過即時問答 0。

那 10 點連唯一買得起的即時問答都沒人去用，所以它現在不是誘餌，是個停車位。
這與 9/05 拍板的沉沒成本設計直接相關，已在 handoff 記明；決定權在使用者。

### 設計取捨

**「卡住」只算 active。** 已到期的人要的是續訂提醒，不是加購提醒。
兩種名單混在一起，兩邊的文案都會寫不對。

**掃描不動 credits_remaining。** 過期不等於點數歸零——續訂時
`commit_paid_entitlement` 會把舊點數帶過去（續訂疊加是既有設計）。
在這裡清等於偷偷沒收他買過的東西。實際資料也支持這個判斷：
待掃的 7 筆裡有 basic 剩 60 點、pro 剩 150 點兩位付費會員。

**不另開新頁。** 會員資料已經在 `/admin/members`，再開一頁只會讓老師
要記兩個地方。改成在既有頁面加篩選與標記。

### 順帶看到的

待掃的 7 筆裡，2 位 trial 是 **30 點一點都沒用就到期**。
所以 trial 漏斗是三段：完全沒開始 → 做一份就停 → 0 轉換。

### 驗證

tsc 過、vitest 374 passed / 2 skipped、next build 過（108 頁）。
排程效果用等效 SQL 確認：第一次掃 7 筆，過期 38–113 天。

### 遺留

- 本機沒有 CRON_SECRET，無法對正式站打 dry-run，部署後要手動確認一次
- 那 6 位還沒有人聯繫，到期日最晚 10/16

### 流年流月改為程式推定；性別與身分拆欄（2026-09-22）

使用者問：「輸入出生年月日時大運就會跟著出現，流月跟著流年異動，後台好像沒得改。」

**查證結果（先講，因為跟直覺相反）**

`lib/yixue/` 沒有任何大運／流年／流月計算，八字只算到四柱＋月令。
但模型**沒有編造**——查最近 6 份報告，每份都提到大運（1～17 次），內容是：

> 八字命理｜出生年月日時、流年｜**大運、流月**｜高｜缺大運與流月，畢業月份判斷降權
> 3日內補齊大運、流年流月及品牌掛牌預定日資料。

模型行為是對的（判節奏要大運，沒有就降權），**問題是系統把自己算得出來的東西
當成「缺少的客戶資料」**，於是付 20 點的人看到的行動方案第一條是叫他自己補大運，
而且每份報告的八字維度都自動降權。這比幻覺難發現，因為報告讀起來很嚴謹。

後台沒得改是因為 `SchoolConfig` 只有 calendar／meihua／liuyao 三段，沒有 bazi 段；
決策書第 327 行自己把「起運法」列在尚未開放的八字階段。**功能不存在，不是漏做設定頁。**

**使用者拍板走 A**：先做流年流月（零爭議、不需簽核），大運等老師定起運法。

**做了什麼**

- `lib/yixue/calendar/tyme.ts` 新增 `fleetingMonthAt()`（tyme4ts 只能在這個檔用）。
  取節氣時刻 +5 天當節月內部探針，避開交節 tie-break 的邊界問題。
- 新增 `lib/yixue/bazi/fleeting.ts`：流年（立春分界，與年柱同一套）＋往後 6 個節月。
  **每個流月各自帶所屬流年**——立春換年，六個月的序列本來就會跨兩個流年，
  綁在結構上就不必靠呼叫端記得處理。這正是使用者說的「流月跟著流年異動」。
  6 個月是為了蓋住行動方案的 30 日與時間節奏的中期；不做成流派欄位
  （顯示範圍不是術數分歧，且沒有能區分選項的 golden case）。
- 基準時刻用**事件／起局時間**不是出生時間——會員問的是「現在這件事」。
  沒有事件時間就回 null 並留 warning，不硬算。
- `renderChartForPrompt` 印出流年與六個流月，並明寫兩句：
  「流年流月為推導結果，不是待補資料；不得在報告中要求會員提供」
  「大運：本系統尚未提供程式排的大運（起運法待老師簽核）…不得要求會員自行提供」。
  第二句是關鍵——沒有它，模型看不到大運就會寫成「請會員補齊」，那不是模型的錯。
- 第二輪 digest 也帶流年與當下流月，否則校核輪看不到。
- **兜底報告同一個 bug 一併修**：原本完整度檢核寫「仍需確認完整四柱、大運、流年」、
  行動方案寫「補齊完整八字、大運資料」。改成流年流月由系統推定、大運未納入並降權。

**性別與身分拆欄（為大運鋪路）**

原本 `genderOptions = ["男","女","其他／不指定","企業主","考生"]` 擠在同一格，
選「企業主」就等於沒有性別；而大運順逆是陽男陰女順排、陰男陽女逆排，非有乾淨男／女不可。
而且 `chart.ts` 根本沒把 gender 傳進引擎、`BirthInput` 也沒這欄位——就算寫好演算法也排不出來。
改成 `genderOptions = ["男","女","不指定"]` ＋ 獨立的 `identity` 自由填寫，
一路貫穿 form-config／input-step／_actions／personas／council schema。
進階頁的性別 select 對不在清單內的舊值仍會列出，不靜默換掉使用者填過的資料。

⚠️ **`gender` 預設值仍是「男」，大運上線前必須重新檢視**：沒被動過的預設會讓
一半的人靜默拿到錯的大運順逆。已在 `_form-config.ts` 該行加註。

**驗證**

tsc 過、vitest 41 檔 **388 passed** / 2 skipped、build 108 頁。
新增 `fleeting.test.ts` 9 case（含跨立春換流年、立春前後一刻、長度 0 保底）、
`chart.test.ts` 5 case（含「prompt 明說不得要求會員提供」「大運未實作必須明講」）。
prompt-baseline 快照刻意更新 5 則（個案資料區塊 3、兜底報告 2），測試檔頭已補紀錄。
另修掉自己在 prompt 裡寫的 `**` 粗體——報告規則禁止 Markdown 粗體，prompt 不該示範被禁的寫法。

**未做**：大運。需要老師先定起運法（三天折一年 vs 按交節時刻精算）與排幾步。

### 修：老師簽了流派，系統卻對客戶說「待簽核」（2026-09-22）

盤點「待老師確認的有哪些」時發現先前的認知已過時：
`ai_school_profiles` 有 **published v2，2026-09-08 12:39，decided_by = 風羿老師**。
那筆躺了一個月的草稿他發布了，而且多選了梅花與六爻：
...
**但簽核資訊沒有生效。** 後台的「決定人」存進 DB 欄位 `decided_by`，
引擎與後台橫幅讀的卻是 `settings.decidedBy`（空字串）。後果：
- 後台橫幅顯示「尚未經老師簽核」
- **每一份收費報告對客戶印「採用流派：風羿老師流派 v1（暫定，待簽核）」**

與先前那批同一種病（後台顯示 ≠ 實際狀態），方向相反：這次是老師做了、系統說他沒做。

**修法**
- `lib/school-settings/load.ts` 新增 `mergeSignature()`，讀取時把 DB 欄位補進 settings。
  **在讀取時合併而不是改資料**：已發布的舊版本立刻就對，不必動任何一列。
  settings 已有值時不覆蓋（那是更晚寫入的來源）。
- publish API 一併把簽核寫進 settings，讓新版本自帶完整資訊。
- `renderChartForPrompt` 新增第三參數 signature，**簽核狀態獨立印一行**：
  流派名稱是老師自己打的字，他簽核後未必會回去把「（暫定，待簽核）」拿掉，
  靠名稱判斷會讓已簽核的盤一直顯示待簽核。未簽核時印
  「尚未簽核，本次判讀請據此說明限制」。

新增 `signature.test.ts` 6 case（含用實際事故資料重現）、`chart.test.ts` 2 case。
tsc 過、vitest 42 檔 396 passed / 2 skipped、build 108 頁。

### 後台能力盤點（回答「老師可以在後台維護哪些」）

**已經能自己改**
| 區塊 | 頁面 | 資料量 | 老師用過 |
|---|---|---|---|
| 排盤流派決策 1–7 | `/admin/school-settings` | v2 已發布 | ✅ 9/8 |
| 文件庫（最直接的 prompt 補充） | `/admin/documents` | 3 份 20,525 字 | ✅ 9/8 上傳兩份反證規則 |
| 報告內容（人設／品質門檻／骨架／反證） | `/admin/prompt-settings` | **0 筆，從沒發布** | ❌ |
| 面相十二宮主輔部位 | `/admin/face-rules` | 1 筆 | 有資料 |
| 面相判讀規則 | `/admin/face-teachings` | **67 條** | 有資料 |
| 面相知識庫 | `/admin/face-knowledge` | **28 張卡**，含匯入匯出 JSON、版本紀錄、四階段狀態 | 有資料 |

**沒有入口**
1. 決策 8 奇門定局法（拆補／置閏／茅山）＋ 轉盤／飛盤 —— `SchoolConfig` 無 qimen 段
2. 起運法（大運）—— 無欄位，功能也未實作
3. 面相報告骨架／prompt —— 寫死在 `lib/face-analysis/report.ts`，
   沒有等同 council `prompt-settings` 的可編輯層
4. 面相覆核題 `face_review_questions`（5 筆）—— 有 API 沒頁面
...
## 2026-09-23（續）｜Gemini 失敗退避與告警：調查與設計（未動工）

### 使用者交辦

「那個 Gemini 失敗要做退避跟告警」，隨後追問「現在判斷決策報告的 API 模型是哪一家？」，
最後說「先這樣」。**本輪只完成調查與設計，沒有寫任何程式。**

### 調查

先查正式庫的實際失敗分布，不憑印象設計。失敗紀錄存在
`council_runs.first_round` / `debate_round` 的 ModelResult 陣列裡：

- Gemini「high demand」**11 次**（最近一次 2026-09-23 01:13，就是我當天實跑那兩份之一）
- Gemini 逾時 2、DeepSeek 逾時 2、OpenAI 逾時 1

關鍵在於：**這些都是 `withRetry()` 重試過之後仍然失敗的次數。**
現有重試是零延遲的，對「上游壅塞」這種需要時間才會好的錯誤幾乎無效。

### 重要判斷

**1. 退避不能無腦加 sleep，預算只剩 10 秒。**
最壞情況 R1(45×2) + R2(45×2) + 終稿(110×1) = 290 秒，`maxDuration` 是 300。
可行的理由是「high demand」屬於**秒回**的錯誤而非逾時——第一次 2 秒失敗就還剩 43 秒。
所以退避必須**預算感知**：睡之前先算 `elapsed + backoff + timeoutMs <= budgetMs`，
塞不下就不睡。先確認限制再設計，避免做出一個會讓報告整個逾時的「改善」。

**2. 只做退避不做錯誤分類，等於把浪費拉長。**
現在除了訊息含「未設定」以外什麼錯都重試，400 這種永久性錯誤也會白花 45 秒。
`ModelResult` 要先加 `status`，才有辦法區分「該退避重試」（429／5xx／逾時／網路中斷）
與「重試也沒用」（400／401／403）。

**3. 查告警管道時查到一個更嚴重的問題。**
`npx vercel env ls production` 的清單裡**沒有 `RESEND_API_KEY`**。
`sendAdminAlert()` 缺 key 時直接回 `skipped: missing_resend_config`——
**推論：全站 admin 告警一封都沒發出去過**，包含綠界付款異常、註冊異常、pending-drafts 排程。
這已經超出 Gemini 的範圍：付款出事沒有人會知道。已寫進 handoff 的起手式，
並標註優先度應高於 Gemini 退避本身。

**4. 因此告警設計成兩層，不綁死在寄信上。**
後台可見的 provider 健康面板（今天就有效，資料來源是上述 SQL）＋
沿用 `sendAdminAlert()`（Resend key 設好後自動生效，不必再改程式）。
只做寄信等於做了一個今天不會響的鈴。

### 順帶回答：決策報告接哪幾家

一份報告打 **7 次 API、橫跨三家**：第一輪平行三家（OpenAI 主判讀／Gemini 策略推演／
DeepSeek 攻防反證）、第二輪攻防同三家各一次（`ENABLE_DEBATE_ROUND` 可關）、
**終稿只由 OpenAI 寫**。

正式站環境變數清單確認：有 `OPENAI_MODEL`（本機是 `gpt-4.1-mini`；正式站的值未讀，
要讀就得把整包 secret 拉到磁碟，沒有這樣做），**沒有** `GEMINI_MODEL` 與 `DEEPSEEK_MODEL`
→ 兩者走程式預設 `gemini-2.5-flash`（`thinkingBudget: 0`）與 `deepseek-chat`。

因為終稿只靠 OpenAI，Gemini 掛掉報告照樣生得出來，只是從三家意見變兩家——
**會員付一樣的錢、拿到少一家意見的報告，而且沒有任何跡象。** 這就是要處理的靜默降級。

### 產出檔案

無程式改動。本次只更新 `handoff.md`（新增補記 09:35 與起手式）與本檔。

### 驗證結果

不適用（未改程式）。調查用的 SQL 與 `vercel env ls` 輸出已記在 handoff 補記中，接手者可重跑。

### 遺留事項

1. Gemini 退避＋告警**整項未動工**，設計已定案，見 handoff 補記（09:35）。
...
## 2026-09-23（續五）｜回答「決策系統能不能交給會員」：逐層實測

### 使用者的問題

「現在的決策系統是正常的嗎？能夠正常交給會員使用嗎？會不會又出現大運資料不出現的情況？」

### 查到的缺口（已修 `c0c7afd`）

**會，而且是我自己留下的。** 2026-09-22 性別預設「男」→ 人人都有大運，
但女性會員沒改就拿到男性的大運。隔天改成不預選是對的方向，
**但只加提示、沒擋送出**——缺口從「拿到錯的大運」變成「拿不到大運」，
會員照樣扣 20 點拿回一份沒有大運的報告。對他一樣是壞的。

### 重要判斷

**1. 改了預設值就要一路想到「沒填會怎樣」。**
昨天只做了一半：拿掉危險的預設，卻沒補上必填檢查。
危險的預設與缺少檢查是同一個問題的兩面，分兩天做就會留下空窗。

**2. 掃描的覆蓋範圍要對著風險設計，不是對著方便設計。**
原本的全時段掃描出生日固定在 1980-05-05，只變起卦時刻——
看起來掃了 1920 張盤，但「不同生日排不排得出大運」一次都沒驗到。
而大運是八字裡唯一會去翻節氣表的環節，八字那段又沒有 try/catch，
某個生日讓它拋錯就是整張盤變 null、連四柱都沒有。已補 2610 張盤的掃描。

**3. 「盤裡有」不等於「會員看得到」。**
使用者最初的抱怨就是這個。所以最後一層要查 `final_text` 本身，
確認模型真的把大運寫進報告、而且沒有反過來跟會員要大運。三份都通過。

**4. UI 層必須真的用瀏覽器看。**
今天已經犯過兩次只在視覺上才顯現的錯（CSS 放錯檔、借用不存在的 `.btn` class）。
這次用真實 token 在正式站走了一次會員流程，確認金色必填標記有渲染、
三顆按鈕都沒預選、按下送出真的被擋且沒扣點。

### 產出檔案

- `app/member-ai/decision/page.tsx`：送出前的性別檢查
- `app/member-ai/decision/_steps/input-step.tsx`：`#councilGender` 錨點、「（八字必填）」
- `app/member-ai/decision/decision.css`：`.xf-required`
- `lib/yixue/sweep.test.ts`：大運全出生時段掃描（2610 張盤）＋ 男女方向相反

### 驗證結果

- tsc exit 0；vitest 490 passed / 2 skipped；build 113 頁
- 引擎：2610 張盤全部排得出大運，首運年份皆合理
- 正式站 UI：關卡生效，未扣點
- 報告正文：最近三份都寫出實際大運（例「現行大運癸酉（28-37歲）」），
  沒有一份反過來要求會員提供大運
