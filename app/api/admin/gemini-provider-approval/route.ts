...
const approvalSchema = z.object({
  ...
  regionLabel: z.string().trim().min(1).max(160),
  ...
  const combined = `${value.projectLabel}\n${value.regionLabel}\n${value.note}`;
  if (/AIza[A-Za-z0-9_-]{20,}|bearer\s+[A-Za-z0-9._-]{12,}/i.test(combined)) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await createSupabaseAdminClient()
      ...
      .upsert({
        provider: "gemini",
        organization_label: input.projectLabel,
        project_label: input.regionLabel,
    ...
    await writeAdminAudit({
      ...
      action: "gemini_face_provider_zdr_attested",
      targetType: "face_provider_approval",
      targetId: data.id,
      metadata: { provider: "gemini", projectLabel: input.projectLabel, regionLabel: input.regionLabel, approvedAt: input.approvedAt }
    });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    await writeAdminAudit({
      adminUserId: adminAuth.profile.id,
      action: "gemini_face_provider_zdr_revoked",
      targetType: "face_provider_approval",
      targetId: data?.id || "gemini"
    });
