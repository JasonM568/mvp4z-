...
三、內容規則
1. 本次只啟用以下術數：${enabledTermNames(input.yixue?.modules).join("、")}。報告只能分析並輸出這些術數，未啟用的術數一律不得出現，也不要寫「資料不足無法判斷」之類占位內容。
2. 不得寫空話，例如：低風險推進、調整作息、注意文件，除非明確說明怎麼做、何時做、做到什麼標準。
3. 每一個啟用術數都必須有自己的獨立論述，不可只寫綜合結論。
4. 每一個啟用術數至少要包含：資料輸入、推理過程、初步判斷、風險點、可行策略、本術數小結。
...
export function buildFinalFormatPrompt(enabledTerms: string[]) {
  const terms = enabledTerms.length ? enabledTerms : ["八字命理"];
  const termList = terms.join("、");
  let n = 0;
  const num = () => CJK_NUM[n++] || `第${n}`;

  const head1 = `${num()}、個案總論
1. 最終決策建議：可進、可試行、暫緩、不建議、或補資料後再判。
2. 最大機會。
3. 最大風險。
4. 本案建議採取的操作節奏。`;

  const head2 = `${num()}、術數資料完整度檢核
請用純文字表格呈現（只列本次啟用的術數，欄位以全形｜分隔）：
...
停損條件：`;
  ...
  const finalRec = `${num()}、最終建議
請用五句話內做決策收束，必須具體，不可空泛。`;

  const disclaimer = `${num()}、專業聲明
...
二、本次只啟用以下術數：${termList}。
正式報告「只能」分析並輸出這些術數，絕對不要出現未啟用的術數（未啟用者一律不提、不要寫「資料不足無法判斷」之類占位內容）。每一個啟用術數都要依：資料輸入、推理過程、初步判斷、風險點、可行策略、本術數小結 完整輸出。

三、正式報告格式（嚴格依下列段落與順序，中文序號照抄）：
