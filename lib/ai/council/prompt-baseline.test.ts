...
//
// 刻意更新紀錄：
// 2026-09-05 —— 報告骨架段落順序改成風羿老師《四象問天機｜綜合判讀與回應規則》
//   第五節的順序，並新增「關鍵點」「時間節奏」「關鍵風險」三段。
//   原因：那份文件已納入報告 prompt，但它的段落順序與骨架不同，而骨架寫的是
//   「嚴格依下列段落與順序」，等於同時給模型兩張互相矛盾的版型。以老師的文件為準。
//   受影響 snapshot：最終定稿分身、報告骨架四則。
// 2026-09-05（同批）—— 決策型態依同一份文件第十一節，從五種擴成七種
//   （新增「宜借力推進」「宜調整策略後再進」，其餘五種換成文件的用詞）。
//   受影響 snapshot：品質門檻三則、報告骨架四則。兜底報告未動。
// 2026-09-07 —— 報告骨架新增「反證」段，掛在四象合參之後、時間節奏之前。
//   原因：使用者要求把反證變成看得見的報告條件。反證原本只跑在第二輪攻防（內部），
//   產出從不出現在交付的報告裡，讀者看不到反面論證。老師可在後台
//   「報告內容維護 → 反證」自行編輯要反證什麼，body 會原文進入 prompt。
//   與「關鍵風險」的分工刻意寫進預設內容：風險是執行面可能出什麼事，
//   反證是這個判斷本身可能就是錯的。
//   受影響 snapshot：報告骨架四則。兜底報告與品質門檻未動，交付門檻的結構錨點也未動
//   （那是「≥4 個錨點」的寬鬆檢查，加進去只會讓門檻更鬆）。

// 2026-09-22 —— 性別與身分拆成兩欄（原本是同一格「性別／身份」）。
//   原因：原清單是男／女／其他／企業主／考生，選「企業主」就等於沒有性別，
//   而大運順逆是陽男陰女順排、陰男陽女逆排，沒有乾淨的男／女排不出來。
//   受影響 snapshot：個案資料區塊三則（只有那一行，其餘位元組不變）。
//   同批新增流年流月進排盤區塊，但那是 renderChartForPrompt 的輸出，
//   本檔的 snapshot 不涵蓋，故不受影響。
// 2026-09-22（同批）—— 兜底報告三句刻意改寫：原本把流年、大運列為「待會員補齊」，
//   完整度檢核寫「仍需確認完整四柱、大運、流年」、行動方案寫「補齊完整八字、大運資料」。
//   流年流月是系統推導得出的，不該跟付費會員要；大運則是未實作，
//   要說「未納入並降權」而不是叫會員自己提供。受影響 snapshot：兜底報告兩則。
