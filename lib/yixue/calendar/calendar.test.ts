...
describe("時柱五鼠遁自算", () => {
  // 顯式 timeout：這條單獨跑約 2.4 秒，但與 tsc 或 build 併跑時會撞破
  // vitest 的 5 秒預設而假性失敗（2026-09-23 實際發生過一次）。
  // 正確的處置是放寬時限，不是縮小掃描範圍——
  // 閃爍的測試比沒有測試更糟，因為它教人忽略紅燈。
  it("[交叉] 預設流派下與 tyme4ts 完全一致（掃描全年每日每個時辰）", { timeout: 30_000 }, () => {
...
describe("[性質] 大範圍掃描", () => {
  ...
  // 正確的處置是放寬時限，不是縮小掃描範圍——
  // 閃爍的測試比沒有測試更糟，因為它教人忽略紅燈。
  it("日柱在 1900–2100 逐日恰好前進一個甲子，不跳號不重複", { timeout: 30_000 }, () => {
